...
export const cartService = {
    ...
    addByName: (productName: string, qta: number, sessionId?: number | null) =>
        apiFetch('/cart/add-by-name', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ product_name: productName, qta, session_id: sessionId }),
        }),

    addByCode: (codArt: string, qta: number, sessionId?: number | null) =>
        apiFetch('/cart', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'add', cod_art: codArt, qta, session_id: sessionId }),
        }),
};
